...
NnKvCacheSlice sliceKvCache(NnUint kvDim, NnUint seqLen, NnUint nNodes, NnUint spSize, NnUint spRank) {
    NnKvCacheSlice s;
    assert(kvDim % nNodes == 0);
    assert(spSize >= 1);
    assert(seqLen % spSize == 0);
    s.kvDim0 = kvDim / nNodes;
    s.localSeqLen   = seqLen / spSize;
    s.localSeqStart = spRank * s.localSeqLen;
    // Buffer always holds full seqLen — each rank writes only its local slice,
    // then SYNC_SP_KV allgather fills the full buffer across the SP group.
