...
### 노드당 실효 연산 (실측, 2026-08-04 갱신)

Cortex-A76 4스레드, Q40 weight × Q80 activation, `bench_sgemm.cpp`:
...
> 초기 가정이던 30 GFLOPS는 [빌드 설정 버그](02-baseline-dotprod-fix.md) 때문에 나온 값이었다.
> **정정된 상한은 ~110 GFLOPS.** 연산 시간이 3.7배 줄어드는 만큼 통신/연산 비율이
> 그만큼 나빠지므로, 아래 통신량 표의 상대적 심각도는 **커진다**(H1에 유리).
>
> 또한 batch 32에서 포화하므로, **prefill 청크 크기는 연산 효율이 아니라
> 통신/파이프라인 관점에서만 결정하면 된다.** 설계 공간이 그만큼 좁아진다.
