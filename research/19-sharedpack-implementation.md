...
- 성능 측정이므로 검증기는 끈다. 정확성은 §8.5 와 `results_correctness.tsv` 가 별도로 담당
- harness `prefill_bench/anchor.sh`, artifact `artifacts/n8_ab/results_anchor_valid.tsv`

### 9.2 결과 — 독립 두 세션에서 재현

| 세션 | 바이너리 | 커밋 | 쌍 비율 | R |
|---|---|---|---|---|
| 1 | `0c3accb8` | `190a471` | 1.4355 1.4654 1.4411 1.4410 | 1.4457 |
| 2 | `a6c22236` | `4468558` | 1.4481 1.4107 1.4426 1.4033 | 1.4260 |

**통합 8쌍 R = 1.4358, 범위 1.4033~1.4654, log-SD ±1.4%**

세션 2(최종 출하 바이너리) 상세:

```
A (BASE)  기하평균 7,611.9 ms   범위 3.0%
B (SP)    기하평균 5,337.8 ms   범위 4.0%
```

**정확성: 9회 전부 `c52e50e37e65`** — 같은 wave 조건의 BASE 와 동일.
...
이 1.436× 를 DerivePP 의 4.57~4.68× 에 곱해 새 성능을 주장하면 **안 된다.**
두 측정의 조건·베이스라인·세션이 다르다. 최종 시스템 대 llama.cpp 는
동일 조건에서 직접 다시 재야 한다.

### 9.5 측정 변동에 대한 정정

이전에 "이 머신의 E2E 변동은 최대 51%" 라고 적고 그것을 근거로 판단을 미뤄왔다.
통제된 조건에서는 BASE 3.0~3.5%, SP 2.1~4.0% 이고, 쌍 비율의 log-SD 는 ±1.4% 다.
51% 는 조건이 통제되지 않은(워커 재기동 없음, 세션 혼재) 관측이었다.
`research/20` 에 정정을 반영했다.
