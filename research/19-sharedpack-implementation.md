...
## 8. 분산 검증 — SharedPack 은 wave 없이는 정확하다 (2026-08-25)

### 8.1 결론

`N=8` 에서 SharedPack 이 로짓을 바꾸는 원인은 **분산이 아니라 wave 파이프라이닝**이다.

| 구성 | SP | prefill ms | logits md5 |
|---|---|---|---|
| N=1 (wave 무관) | 0/1 | — | `8b8178a50a97` |
| N=8 wave **off** | 0 | 35,505 | `8b8178a50a97` |
| N=8 wave **off** | 1 | 27,621 | `8b8178a50a97` |
| N=8 wave **on** | 0 | 7,799 | `c52e50e37e65` |
| N=8 wave **on** | 1 | 5,147 | `895c8b182e9c` |
| N=2 wave **on** | 0 | 20,147 | `46edf541991a` |
| N=2 wave **on** | 1 | 14,812 | `1e56b8e5471c` |

**wave off 에서 SP=0 과 SP=1 의 로짓이 비트 단위로 같다.** 8노드 분산·PP 경계·
q80 전송을 모두 통과한 뒤에도 동일하다. 따라서 `packQ80x4Forward` 의 레이아웃과
`matmulForward_repack` 의 prepacked 읽기는 분산 환경에서 **exact** 다.

`ppSize=2` 에서 이미 wave 불일치가 나타난다. 8노드 특유의 문제가 아니다.

### 8.2 부수 정정 — wave 는 BASE 로짓도 바꾼다

`N=8` wave off BASE (`8b8178a50a97`) 는 `N=1` 과 같은데 wave on BASE 는
`c52e50e37e65` 다. 즉 **wave 자체가 SP 와 무관하게 수치를 바꾼다.**

`research/17` §7.7.3 에 "`N=1` 과 `N=8` 의 로짓 차이는 stage 경계 q80 왕복 때문"
이라고 적었는데, 원인 귀속이 틀렸다. 분산만으로는 로짓이 바뀌지 않는다.
바뀌는 것은 wave 경로이며, wave 는 `forwardPrefillNoWait()` 로 stage 활성화를
`pipelineActivationType`(F_Q80)으로 보낸다. 비-wave 경로와 전송 경로가 다르다.

이 차이는 SP 도입 이전부터 있던 것이므로 SharedPack 의 결함이 아니다.
다만 **wave-on BASE 를 정확성 기준선으로 쓰면 안 된다** — 기준선 자체가
비-wave 와 다르기 때문이다.

### 8.3 편차 크기 (N=8 wave on, BASE 대비)

| 모드 | maxabs | rms | top1 일치 |
|---|---|---|---|
| 2 (Down 만) | **NaN** | NaN | 아니오 |
| 3 (y_pack 만) | 17.20 | 3.58 | 아니오 |
| 1 (전체) | 18.70 | 4.05 | 아니오 |

`NaN` 은 미초기화 메모리를 읽는다는 신호다. 미묘한 재정렬이 아니다.

### 8.4 진행 중인 판별

`DLLAMA_SHARED_PACK=4` 를 추가했다 — **pack op 은 삽입하되 matmul 은
`NN_NO_PREPACK` 을 쓰게 한다.** 모드 4 가 모드 0 과 다르면 원인은 pack 데이터가
아니라 **그래프 변경**(op 개수 변화가 wave 스케줄/세그먼트 경계에 미치는 영향)이다.
같으면 원인은 pack 버퍼 내용이다.

### 8.5 근본 원인 — 축 규약 오독 (확정, 수정됨)

`DLLAMA_VERIFY_PACK=1` 계측(matmul 이 공유 버퍼와 자기가 다시 만든 pack 을 대조)이
`inY=32` 를 찍었다. K(4096)가 아니라 **배치 행 수**였다.

```
NnSize3D size2D(NnFloatType floatType, NnUint y, NnUint x)   // nn-core.cpp:118
```

**첫 인자가 y(행 수), 둘째가 x(폭)** 이다. 여기서 결함 두 개가 나왔다.

| # | 위치 | 잘못 | 결과 |
|---|---|---|---|
| 1 | `packQ80x4Forward` | `kElems = inputSize.y` | K 대신 행 수(32)를 읽어 `kBlocks = 1` |
| 2 | `llm.cpp` d_pack | 폭을 `w2Slice.d`(=4096, **출력**) 로 | Down 의 K 는 `w2Slice.n0`(=14336). 버퍼가 3.5배 작음 |

결함 1 은 `firstDiffByte=2` 를 정확히 설명한다. `kBlocks=1` 이면 pack 이
`in[g*4*1]` 을 읽어 **row 0 의 block 0,1,2,3 을 rows 0,1,2,3 으로 착각**한다.
`scale[0]`(byte 0-1)은 우연히 맞고 `scale[1]`(byte 2-3)부터 어긋난다.

결함 2 는 mode 2 의 `NaN` 을 설명한다 — 버퍼 밖 미초기화 메모리를 fp16 스케일로 읽었다.

**수정 후 재검증** (N=2, `--pp-layers 16,16`):

| wave | SP | logits md5 | BASE 일치 | VERIFY 히트 |
|---|---|---|---|---|
| 1 | 1 | `46edf541991a` | 예 | 0 |
| 0 | 1 | `8b8178a50a97` | 예 | 0 |

root/worker 양쪽 모두 히트 0. **SharedPack 은 이제 분산·wave 양쪽에서 exact 다.**

### 8.6 무효화 — §8.1 의 SP 행과 잠정 1.286×

§8.1 표의 **SP=1 행과 §8.5 이전에 적었던 1.286× 는 모두 깨진 빌드에서 나온 것이라
무효다.** BASE 행(`8b8178a50a97`, `c52e50e37e65`, `46edf541991a`)은 SP 와 무관하므로
유효하고, "wave 가 BASE 로짓을 바꾼다"는 §8.2 의 정정도 유효하다.

성능은 수정된 바이너리로 앵커 설계를 거쳐 다시 재야 한다. 아직 어떤 배수도 주장하지 않는다.

### 8.7 가장 중요한 교훈 — 비트 동일 통과가 무죄를 뜻하지 않았다

수정 전 `N=8 wave-off` 와 `N=1` 에서 SP=1 의 로짓이 BASE 와 **비트 단위로 같았다.**
그 상태에서 pack 은 `kBlocks=1` 로 쓰고 있었고 Down 버퍼는 3.5배 작았다.
즉 **end-to-end 해시 일치가 커널 계층의 심각한 결함을 통과시켰다.**
(비트 동일이 어떻게 성립했는지는 규명하지 못했고, 수정 후에는 무의미해졌다.)

교훈: **출력 해시는 커널 정확성의 검증기가 아니다.** 커널이 소비하는 중간 표현을
직접 대조하는 계측이 있어야 한다. `DLLAMA_VERIFY_PACK` 은 유지한다.

### 8.8 새 실수

| 실수 | 대응 |
|---|---|
| 워커 실행 중 `scp` → `ETXTBSY` ("dest open Failure") | 배포 전에 `pkill -x dllama` 먼저 |
| `--steps 2` 로 단축 시도 → "prompt tokens > steps" 로 전량 실패 | `--steps` 는 프롬프트 토큰 수보다 커야 한다. 453 이 이미 최소 |
| `size2D` 축 규약을 확인 없이 가정 | 선언을 읽는다: `size2D(t, y, x)`. 슬라이스 필드도 `n0`/`d` 가 무엇인지 matmul 소비 지점에서 역추적 |
| E2E 해시 일치를 커널 정확성의 증거로 사용 | 중간 표현 대조 계측을 먼저 만든다 (§8.7) |

---

## 9. 확정 성능 — N=8 앵커 측정 (2026-08-25, 빌드 `0c3accb8`)

### 9.1 설계

- 프롬프트 512 토큰, `--pp-layers 8,8,8,8,8,8,8,8`, wave on, `--n-batches 32 --prefill-chunk-size 32`
- 순서 `0,0,1,1,0,0,1,1,0` — **1번은 웜업으로 폐기**. 정/역 교차로 세션 드리프트 상쇄
- 매 실행마다 워커 7대를 재기동해 상태 잔여 제거
- 쌍 비율 = (시간상 앞 A · 뒤 A 의 기하평균) / B, 그 기하평균을 R 로 보고

### 9.2 결과

| idx | mode | prefillMs | syncWaitMs | logits md5 |
|---|---|---|---|---|
| 1 | BASE | 7355.04 | 3402.45 | `c52e50e37e65` (웜업, 폐기) |
| 2 | BASE | 7383.38 | 3576.75 | `c52e50e37e65` |
| 3 | SP | 5216.94 | 2237.74 | `c52e50e37e65` |
| 4 | SP | 5110.66 | 2037.85 | `c52e50e37e65` |
| 5 | BASE | 7596.13 | 3784.54 | `c52e50e37e65` |
| 6 | BASE | 7342.58 | 3502.14 | `c52e50e37e65` |
| 7 | SP | 5128.46 | 1934.10 | `c52e50e37e65` |
| 8 | SP | 5128.98 | 2018.35 | `c52e50e37e65` |
| 9 | BASE | 7439.12 | 3616.45 | `c52e50e37e65` |

```
A (BASE)  기하평균 7,439.7 ms   범위 3.5%
B (SP)    기하평균 5,146.1 ms   범위 2.1%
쌍 비율   1.4355  1.4654  1.4411  1.4410
R = 1.4457
```

**정확성: 9회 전부 `c52e50e37e65`.** SP 는 BASE 와 비트 단위로 같은 로짓을 낸다.
§8.7 의 교훈대로, 이 해시 일치는 이제 `DLLAMA_VERIFY_PACK` 히트 0 과 함께 있을 때만
의미가 있다.
...
| 성분 | BASE | SP | 비율 |
|---|---|---|---|
| prefill 전체 | 7,440 | 5,146 | **1.446×** |
| syncWait | 3,619 | 2,054 | 1.762× |
| non-wait residual | 3,821 | 3,092 | 1.236× |

**`prefillMs - syncWait` 는 순수 연산 시간이 아니다.** 전송, executor 오버헤드,
잔여 동기화가 섞여 있다. 따라서 "연산 1.236×" 가 아니라 **"non-wait residual
1.236×"** 로 적어야 한다.

논문 표현:

> non-wait residual 은 1.236× 개선됐고 `syncWait` 는 1.762× 감소했다.
> 로컬 연산 경로 개선이 파이프라인의 critical-path stall 까지 줄여
> E2E 1.446× 로 증폭됐다.

non-wait residual 1.236× 가 N=1 operator 누적 1.205× 와 정합한다는 점이
연산 경로가 실제 원인임을 뒷받침한다.

이것은 §16 의 균형용량 수축 보조정리와 같은 방향이다: 파이프라인에서는 스테이지
비용 감소가 대기 시간에 증폭되어 나타난다.

### 9.4 측정 변동에 대한 정정

이전에 "이 머신의 E2E 변동은 최대 51%" 라고 적고 그것을 근거로 판단을 미뤄왔다.
이번 9회 측정에서 BASE 범위는 **3.5%**, SP 범위는 **2.1%** 다.
51% 는 조건이 통제되지 않은(워커 재기동 없음, 세션 혼재) 관측이었다.
**매 실행 워커 재기동 + 웜업 폐기**를 하면 변동은 수 % 수준이다.
`research/20` 의 변동 계층 기술을 이에 맞게 정정해야 한다.
