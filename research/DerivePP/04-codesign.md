# 04. 계층 Co-design — 2×2 Ablation 과 Recurrence 검증
...
필요하다.** → [08-status.md](08-status.md)
...
**1스레드에서 이득이 정확히 0 이다.** 제거할 중복이 없기 때문이다.
단일 커널 마이크로벤치의 `1~3 스레드 1.00~1.03×, 4 스레드 1.68×` 전이가
**N=8 분산 E2E 에서도 그대로 재현된다.**
